...
void PCLLocalization::initializeParameters()
{
  ...
  get_parameter("lidar_type", lidar_type_);
  ...
  RCLCPP_INFO(get_logger(),"lidar_type: %s", lidar_type_.c_str());
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  if (lidar_type_ == "livox") {
    ...
    if (width > 0) {
      ...
      if (timestamp_found) {
        const uint32_t last_point_index = (width - 1) * point_step;
        const uint8_t* data_ptr = msg->data.data();
        const uint8_t* timestamp_ptr = data_ptr + last_point_index + timestamp_offset;
        
        uint64_t timestamp_bits = 0;
        for (int i = 0; i < 8; i++) {
          timestamp_bits |= static_cast<uint64_t>(timestamp_ptr[i]) << (i * 8);
        }
        double absolute_time_ns = *reinterpret_cast<const double*>(timestamp_ptr);
        const uint32_t sec = static_cast<uint32_t>(absolute_time_ns / 1000000000ULL);
        const uint32_t nanosec = static_cast<uint32_t>(absolute_time_ns - sec * 1000000000ULL);
        cloud_stamp = rclcpp::Time(sec, nanosec);
        RCLCPP_INFO(get_logger(), "stamp time %u.%09u last point time: %u.%09u",msg->header.stamp.sec,msg->header.stamp.nanosec, sec, nanosec);
      }
    }
  }
  ...
  if (use_imu_) {
    double received_time = cloud_stamp.seconds();
  ...
  corrent_pose_with_cov_stamped_ptr_->header.stamp = cloud_stamp;
  ...
  map_to_base_link_stamped.header.stamp = cloud_stamp;
  ...
  if (!enable_map_odom_tf_) {
    ...
    try {
      odom_to_base_link_msg = tfbuffer_.lookupTransform(
        odom_frame_id_, base_frame_id_, cloud_stamp, rclcpp::Duration::from_seconds(0.1));
    } catch (tf2::TransformException & ex) {
      RCLCPP_WARN(
  ...
  pose_stamped_ptr->header.stamp = cloud_stamp;
